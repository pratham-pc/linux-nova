...
void nova_init_blockmap(struct super_block *sb, int recovery)
{
	...
	int i;
	int ret;
	int j, blk_node_cnt = 0;

	/* Divide the block range among per-CPU free lists */
	sbi->per_list_blocks = sbi->num_blocks / sbi->cpus;
	nova_dbg("BLCKS: %lu,\t CPUS: %d,\t COUNT: %lu", sbi->num_blocks, sbi->cpus, sbi->per_list_blocks);
	for (i = 0; i < sbi->cpus; i++) {
		free_list = nova_get_free_list(sb, i);
		...
		if (recovery == 0) {
			...
			for (j = 0; j < free_list->num_free_blocks; j+=2048) {
				blknode = nova_alloc_blocknode(sb);
				if (blknode == NULL)
					BUG();
				blknode->range_low = j;
				if (j + 2047 > free_list->num_free_blocks - 1) {
					blknode->range_high = free_list->num_free_blocks - 1;
				} else {
					blknode->range_high = j + 2047;
				}
			...
			nova_dbg("BLOCKMAP INIT FINISHED with %lu number of blocknodes", free_list->num_blocknode);
		}
	...
	int rnd)
{
	...
	if (rnd) {
		...
		strt %= free_list->num_free_blocks;
	}
	if (from_tail == ALLOC_FROM_HEAD) {
		int i;
		temp = &(free_list->first_node->node);
